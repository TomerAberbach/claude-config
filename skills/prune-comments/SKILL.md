...
clauses inside one. Leave the comments worth keeping, and say so when none can
go.
...
- Comments don't keep history; version control does. A comment about what the
  code used to be, will become, or changed is a tombstone: it goes stale as soon
  as the surrounding code changes, and the reader can't trust it. An uncommitted
  change counts too. A comment describing an edit made while iterating
  ...
  result
- Cut the clause, not always the comment. Part of a comment can be worth
  keeping: a real warning followed by a restatement of the next line. Trim the
  dead clause and keep the live one
- When in doubt, keep it. Deleting a comment that stated real intent loses
  knowledge that may be expensive to recover. A comment you don't understand may
  state something the code doesn't show. Investigate or leave it, don't guess it
  away
...
so reason from first principles when none fits cleanly.
...
5. If you renamed or extracted anything, run the covering tests or a typecheck
   to confirm behavior is unchanged. Comment-only deletions need no test run
...
- Tombstones: comments about the past, the future, or the change in progress.
  ...
  `// legacy`. Commented-out code is a tombstone too, so delete it
...
- Section labels that name a step the code should name itself:
  `// validate input`, `// now build the request`. These are a sign to extract a
  named function (see below), not to delete the label
- Caller references in API documentation: a doc comment describes the API itself
  ...
  `// the dashboard relies on this being sorted`) goes stale and is overly
  prescriptive: callers change, the code already shows who calls what, and the
  doc shouldn't presume one caller's use is the API's purpose. Trim the caller
  clause and keep whatever describes the API. If the caller's use reveals a real
...
- API documentation comments that feed docs or IDE hovers, even when redundant
  with the signature, because they serve readers who never see the source. Trim
  a dead clause within one, but don't delete the doc comment. Trim a clause that

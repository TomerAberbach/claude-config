...
1. For each changed function or code chunk, search for code that already does
   ...
   2. Dependencies already installed, listed in the manifest, e.g.
...
1. Same reason to change: if a requirement shifts, every call site must require
   the new behavior. If one caller might need to diverge, it's coincidental
   similarity
2. Precise name: the shared code has a name that describes one job. If the best
   available name is vague (`helper`, `util`, `process`, `handleData`), the
...
5. Worth the cost: the shared code saves more than it costs. Extracting three

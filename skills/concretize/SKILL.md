...
what is actually there, keep the terms the domain itself uses, and drop borrowed
figures of speech that the reader must translate back into meaning.
...
Concretize the target named in the arguments if given; otherwise the code or
prose changed in the current commit shown above; if there are no arguments and
the commit has no changes, ask the user what to concretize and stop.

# Principles

- Name the thing, not a picture of it. A metaphor borrows a word from another
  domain to gesture at meaning, and the reader must translate the picture back.
...
- The domain's own words stay. A term the field uses in its specs and docs is
  not jargon to strip; it is the concrete name. "Socket", "stream", "deadlock",
  "idempotent", "quorum" earn their place even though each began as a metaphor.
  The test: would a practitioner write this word in a precise spec, or only in a
  blog post? Keep the first kind; cut the second.
- Plain over fancy, even within the domain. When two correct terms fit, pick the
  ...
  word when it buys no precision; complexity is worth it only when it names a
  distinction the simpler word would lose.
- Concretize, don't change meaning. Replacing a word must preserve the claim. If
  a metaphor is carrying a real distinction no plain term captures, keep it and
  say why rather than flatten the meaning.

These principles double as the test for a replacement: make one only when the
...
1. Read the whole target once so you understand what it actually refers to
   before renaming anything. For code, note how to run its tests or typecheck.
2. Find the candidates: jargon, imported metaphors, and needlessly fancy terms
   (see "What to replace"). Most words are fine; that's expected.
...
5. Apply the changes:
   ...
   - For an identifier built on a metaphor, rename it. Treat this as a
     ...
     that callers outside the target depend on without restoring the public
     name.
...
7. Report what changed: each term replaced and the concrete word it became, and
   any metaphor you deliberately kept because it is the domain's term or carries
   a distinction no plain word does.
...
- Verbs used as filler. "leverage" is "use"; "surface" (as a verb) is "show" or
  "report"; "unpack" is "explain". Plain verbs say the same thing.
- Fancy words that buy no precision. "utilize" is "use", "facilitate" is "help",
...
- Vague abstractions standing in for a concrete referent. "handle the data", "do
  ...
  what. This often points at a rename or extraction, which you should use
  `/stratify` for.
...
- Domain terms that began as metaphors but are now the field's precise
  ...
  the domain.
- A metaphor that carries a distinction no plain term captures. If replacing it
...
- Terms of art the project already uses consistently, even if you'd pick
  ...
  whole vocabulary needs aligning, then you should use `/ubiquitize-language`.
...
- A test or typecheck breaks after a rename: a reference was missed or the name
  ...
  name.
- A replacement reads less clearly than the metaphor: the metaphor was doing
  real work. Put it back and note it as a deliberate keep.
